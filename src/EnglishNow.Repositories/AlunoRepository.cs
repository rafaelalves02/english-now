...
namespace EnglishNow.Repositories
{
    ...
    public class AlunoRepository : BaseRepository, IAlunoRepository
    {
        ...
        public int? Inserir(Aluno aluno)
        {
            ...
            {
                var query = "INSERT INTO aluno (nome, email, usuario_id) VALUES (@nome, @email, @usuario_id); SELECT LAST_INSERT_ID()";
        ...
        public int? Apagar(int id)
        {
            using (var conn = new MySqlConnection(ConnectionString))
            {
                var query = "DELETE FROM aluno WHERE aluno_id = @aluno_id";
        ...
        public IList<Aluno> ListarPorProfessor(int usuarioId)
        {
            ...
            using (var conn = new MySqlConnection(ConnectionString))
            {
                var query = @"SELECT DISTINCT a.aluno_id, a.nome, a.email, u.usuario_id, u.login, u.senha FROM
                                 aluno a INNER JOIN
                                 usuario u ON a.usuario_id = u.usuario_id INNER JOIN
