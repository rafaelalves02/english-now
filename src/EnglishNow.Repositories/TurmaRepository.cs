...
namespace EnglishNow.Repositories
{
    ...
    public class TurmaRepository : BaseRepository, ITurmaRepository
    {
        ...
        public int? Inserir(Turma turma)
        {
            ...
            {
                string query = "INSERT INTO turma (ano, semestre, periodo, nivel, professor_id) VALUES (@ano, @semestre, @periodo, @nivel, @professor_id); SELECT LAST_INSERT_ID()";
        ...
        public Turma? ObterPorId(int id)
        {
            ...
            using (var conn = new MySqlConnection(ConnectionString))
            {
                var query = @"SELECT t.turma_id, t.semestre, t.ano, t.periodo, t.nivel, t.professor_id, p.nome, p.email FROM
                                 turma t INNER JOIN
                                 professor p ON t.professor_id = p.professor_id
                                 WHERE
                                 t.turma_id = @turma_id";
